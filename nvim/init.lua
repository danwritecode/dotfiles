...
require('lazy').setup({
    ...
    config = function()
      vim.api.nvim_create_autocmd('LspAttach', {
        ...
        callback = function(event)
          ...
          -- Customize LSP hover handler with auto-close on cursor move
          local original_hover = vim.lsp.handlers.hover
          vim.lsp.handlers['textDocument/hover'] = function(...)
            local ret = original_hover(...)
            if ret and ret.winid then
              local hover_win = ret.winid
              local original_buf = vim.api.nvim_get_current_buf()

              -- Close hover when cursor moves in original buffer
              vim.api.nvim_create_autocmd('CursorMoved', {
                buffer = original_buf,
                callback = function()
                  if vim.api.nvim_win_is_valid(hover_win) then
                    vim.api.nvim_win_close(hover_win, {})
                  end
                end,
                once = true,
              })
          ...
          -- Configure hover window properties
          ...
          -- Add keybind to focus floating windows with auto-close
          ...
          map('<leader>ld', show_diagnostics, 'Show [L]SP [D]iagnostics')
  ...
  { -- Autoformat
    ...
    opts = {
      ...
      format_on_save = function(bufnr)
        ...
        local disable_filetypes = { c = true, cpp = true, rs = true, rust = true, vue = true, typescript = true }
        return {
          timeout_ms = 500,
  ...
  { -- Highlight, edit, and navigate code
    ...
    build = ':TSUpdate',
    opts = {
      ensure_installed = { 'bash', 'c', 'diff', 'html', 'lua', 'luadoc', 'markdown', 'vim', 'vimdoc', 'rust', 'vue', 'typescript' },
